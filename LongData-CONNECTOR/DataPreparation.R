...
temporalCurve = ALEMTUZUMAB_DATA %>% dplyr::select_if(grepl("ID|EDSS", names(.))) %>%
	gather("Step","Observation",-ID)
temporalCurve$Step = gsub(temporalCurve$Step,pattern = "(EDSS AT )|(EDSS)",replacement = "")
...
for( i in c("CD4","Treg","Th1","Th17")){
	...
	temporalCurve = ALEMTUZUMAB_DATA %>% rename( ID = Mmonths  )%>%
		gather("Step","Observation",-ID)
	...
	temporalCurve$Time = time[temporalCurve$Step]
	temporalCurve = temporalCurve %>% dplyr::select(ID,Observation,Time)%>%
		dplyr::mutate(Time = ifelse(Time < 5, Time*6, (Time-2)*12 ),Observation = as.numeric(Observation) )
