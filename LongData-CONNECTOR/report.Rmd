...
temporalCurve$TypeRel = sapply(1:length(temporalCurve[,1]),
                               function(x) gsub(pattern = "NA",replacement = "",paste(Feature[x,rel],collapse = "")) )
...
patientsToNotConsider = c("PR0002", "PR0004","ZG0005", "ZG0010", "ZG0017")

TemporalCurve = merge(temporalCurve,
                      Feature, by = "ID")

TemporalCurve$Exp = Data
...
do.call(rbind,
        lapply(temporalCurve %>% 
                 ...
                 unlist(),
               function(i){
                 TemporalCurve_sub = TemporalCurve %>%
                   filter(ID == i)
...
Feature = merge(Feature,
                firstRelapse,all.x = T)

Feature$Exp = Data

temporalCurve = 
  temporalCurve %>% 
  dplyr::select(ID,Time, Observation) %>% 
...
# remove time after third injection
temporalCurve = temporalCurve %>% filter( !(ID == "CA0001" & Time == 72) )
